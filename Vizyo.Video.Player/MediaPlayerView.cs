...
namespace Vizyo.Video.Player
{
    public class MediaPlayerView : TemplatedControl
    {
        ...
        private async Task TryOpenFile()
        {
            ...
            await MediaPlayer.ExecuteCommandAsync([MPVMediaPlayer.PlaylistManipulationCommands.Loadfile, uriString]);

            SetCurrentValue(PlayingProperty, true);
        }

        private async Task TryOpenFile_old()
        {
            if (MediaPlayer == null) return;
            ...
            if (files.Count > 0)
            {
                ...
                //string uriString = "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4";
                //await MediaPlayer.ExecuteCommandAsync([MPVMediaPlayer.PlaylistManipulationCommands.Loadfile, uriString]);
        ...
        //    // 1) DOSYA AÇMA — REAL PATH KULLANMA
        //    // Android’de StorageProvider, file.Path içinde tam "content://..." URI döner.
        //    // mpv bu URI'yi direkt oynatabilir.
